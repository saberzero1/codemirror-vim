...
function escapeKeysForNeovim(keys: string): string {
    let result = '';
    let i = 0;
    while (i < keys.length) {
        if (keys[i] === '<') {
            ...
            if (closeIdx !== -1 && closeIdx - i < 20) {
                ...
                if (/^(?:[CSMAcsma]-)*\w+$/.test(inner)) {
                    result += keys.substring(i, closeIdx + 1);
                    i = closeIdx + 1;
                    continue;
                }
            }
...
export class NeovimClient {
    ...
    async input(keys: string): Promise<void> {
        await this.nvim.input(escapeKeysForNeovim(keys));
